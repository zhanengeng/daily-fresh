...
class GoodsType(BaseModel):
    ...
    image = models.ImageField(upload_to="type", verbose_name="商品类型图片") # upload_to随便写写，因为实际储存时用分布式文件储存:fastdfs
...
class GoodsSKU(BaseModel):
    '''商品SKU模型类：名称，简介，价格，单位，库存，销量，*象征图片，状态（上架/下架），种类ID，spu ID'''
    status_choices = (
        (0,'下线'),
        (1,'上线'),
    )
...
class IndexTypeGoodsBanner(BaseModel):
    '''首页分类商品展示模型类：sku ID（具体商品）,种类ID（分种类显示），展示标识（图片/标题），index'''
